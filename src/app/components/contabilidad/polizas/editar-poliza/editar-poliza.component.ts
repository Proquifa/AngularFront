...
@Component({
    ...
    templateUrl: './editar-poliza.component.html',
    styleUrls: ['./editar-poliza.component.scss']
})

export class EditarPolizaComponent implements OnInit, DoCheck {
    /*
    ...
    */
    ...
    public title = 'Ejemplo de Nombre de Empresa - Cliente - Proveedor';
    ...
    fnFillSelects(opc: number) {
        ...
        switch (opc) {
            case 0:
                ...
                let tipoOpt: OptionClass = {
                    id: this.polizaAux.tipo,
                    texto: (this.polizaAux.tipo === 1) ? 'Ingreso' : (this.polizaAux.tipo === 2) ? 'Egreso' : 'Diario'
                };
                this.defaultItem[0] = Object.assign(new OptionClass(), tipoOpt);
                break;
            case 1:
                let empresaOpt: OptionClass = null;
                empresaOpt = {
                    id: this.polizaAux.empresa.idEmpresa,
                    texto: this.polizaAux.empresa.alias
                };
                this.defaultItem[1] = Object.assign(new OptionClass(), empresaOpt);
                break;
            case 2:
                let tipoObjetoOpt = (this.polizaAux.tipo === 1) ? this.lstNCliProv[1] :
                ...
                this.defaultItem[7] = Object.assign(new OptionClass(), tipoObjetoOpt);
                break;
            case 3:
        ...
        if (opc === 4) {
            ...
            setTimeout(() => {
                opc++;
                this.fnFillSelects(opc);
            }, 500);
        }
    }
    fnGetPoliza() {
        this.serviceCatalogo.obtenerPoliza(this.idPoliza).subscribe(resp => {
            console.log('obtenerPoliza', resp.current);
            if (resp.current.idPoliza > 0) {
                let pol = resp.current;
                let fechaAux = pol.fecha.split('-');
                pol.fecha = new Date(Number(fechaAux[0]), Number(fechaAux[1]) - 1, Number(fechaAux[2]));
                switch (pol.tipo) {
                    case 1:
                        this.title = pol.cliente.razonSocial;
                        break;
                    case 2:
                        this.title = pol.proveedor.razonSocial;
                        break;
                    case 3:
                        this.title =
                ...
                for (let item of pol.lstPPoliza) {
                    let nivel = null;
                    let nivel2 = null;
                    let nivel3 = null;
                ...
                this.poliza = Object.assign({}, pol);
                this.polizaAux = Object.assign({}, pol);
            }
            this.fnFillSelects(0);
        }, error => {
            this.fnFillSelects(0);
